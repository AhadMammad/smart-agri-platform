...
# Basemaps for the deck.gl charts. Superset 6 renders a tile URL through
# deck.gl's own TileLayer and only mounts mapbox-gl for a `mapbox://` style, so
# an OSM tile URL needs no Mapbox account and `MAPBOX_API_KEY` stays unset.
#
# Restricted to OSM deliberately: 6.x's default list also offers several
# mapbox:// styles, and picking one silently reproduces the blank basemap this
# replaced — mapbox-gl v2+ validates its token server-side and, on the 401,
# permanently blanks the canvas.
#
# tile.openstreetmap.org is already in Superset 6's own CSP `connect-src`;
# pointing this anywhere else means extending TALISMAN_CONFIG too.
DECKGL_BASE_MAP = [
